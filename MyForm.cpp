...
void SIPclient::MyForm::CallBtn_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    sprintf(sip_target_url, "sip:%s@%s", app_cfg.phone_number, app_cfg.sip_domain);//;transport=tcp
...
void SIPclient::MyForm::RegisterBtn_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    sprintf(sip_provider_url, "sip:%s", app_cfg.sip_domain);//;transport=tcp
...
void SIPclient::MyForm::start_sip_stack()
{
    ...
    tcfg.port = 5060;
    status = pjsua_transport_create(PJSIP_TRANSPORT_UDP, &tcfg, &tcp_tp_id);
